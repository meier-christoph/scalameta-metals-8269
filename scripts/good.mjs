#!/usr/bin/env node
/**
 * Go-to-definition on LOCAL symbol HelloConfig — should succeed even without
 * waiting for indexing, because Metals falls back to a guess-based lookup.
 *
...
const relPath = "src/main/scala/org/example/Main.scala";
...
// Line 6: `val cfg = HelloConfig(verbose = true, name = "John Doe")`
const line = 5;       // 0-indexed
const character = 14; // start of "HelloConfig"
const symbol = "HelloConfig";
...
// ── Start Metals ────────────────────────────────────────────────────────────
console.log("=== go-to-definition on LOCAL symbol ===");
...
// ── 3. Fire definition request IMMEDIATELY (no waiting for indexing) ────────
const defId = ++nextId;
console.log(`Sending definition request immediately (no wait for indexing)...`);
send({
  jsonrpc: "2.0", id: defId, method: "textDocument/definition",
  params: { textDocument: { uri: fileUri }, position: { line, character } },
});

// ── 4. Read messages until we get the definition response ───────────────────
for (;;) {
  const msg = await read(120_000);
  if (!msg) { console.log("Timeout waiting for definition response."); break; }

  // Answer server-initiated requests
